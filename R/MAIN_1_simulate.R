...
config.file.dir <- here("simulation_config.txt") #' ../Users/philipwaggoner/Desktop/input_data/simulation_config.txt'
...
# Clean up
rm(
  raw.data, this.mask, wind.unit, WS, WD, row.list,
...
for (s in 1:n.s) {
  ...
  big.C.list <- foreach(h = 1:n.chunks) %dopar% {
    ...
    this.chunk.end <- min(
      h * standard.chunk.size,
      n.ints
    )

    # Last index of the extended chunk to capture stragglers
    ext.chunk.end <- min(
      h * standard.chunk.size + cutoff.t * 60 / dt - 1,
      n.ints
    )
    ...
    for (j in 1:length(this.chunk.mask)) {
      ...
      for (k in 1:(cutoff.t * 60 / dt)) {
        ...
        if (min(distances) > ignore.dist) {
          next
        }

        # Compute the concentration at each receptor location from the kth puff
        this.C <- this.C + dt * gpuff(
          Q = Q.truth[this.chunk.mask][j],
    ...
    if (h != n.chunks) {
      ...
      for (j in seq(length(this.chunk.mask) + 1, length(ext.chunk.mask))) {
        ...
        for (k in 1:(cutoff.t * 60 / dt)) {
          ...
          if (col.it > length(this.chunk.mask)) {
            next
          }
...
#saveRDS(data.to.save, output.file.path) # can substitute write_rds + here if really needing to write out
